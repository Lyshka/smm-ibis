...
const PriceBlock = ({ priceByn, priceRub, services, title, desc, hide=true }) => {
  ...
  return (
    <div className="w-[317px] rounded-2xl shadowPrice">
      <h3 className="px-6 py-4 text-sm">{title}</h3>
      ...
      <div className="bg-[#FF3A2D] p-6 text-white w-full">
        <div className="flex text-lg flex-col gap-y-8">
          {hide && (
            <div className="flex text-lg flex-col gap-y-8">
              <p className="font-bold text-center">
                от <span className="font-black text-[56px]">{priceByn}</span>{" "}
                BYN
              </p>

              <p className="font-bold text-center">
                от <span className="font-black text-[56px]">{priceRub}</span>{" "}
                RUB
              </p>
            </div>
          )}
          ...
          <p className="font-semibold leading-6 w-[280px]">
            Продвижение на маркетплейсах Wildberries, ЯндексМаркет, OZONи д.р.
          </p>

          <p className="font-semibold leading-6 w-[280px]">{desc}</p>
        </div>
      </div>
